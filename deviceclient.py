...
class Notifications:
    ...
    _NOTIFICATION_TEMPLATE = {
        "record_version": _NOTIFICATION_VERSION,
        "type": "info",
        "message": "",
        "RGB_LED": "white"
    }
    ...
    def _verify_RGB_list_input(RGB_LED):
        if len(RGB_LED) != 3:
    ...
    def _verify_notification_inputs(self, device, message, RGB_LED, qos):
        ...
        if not (isinstance(RGB_LED, list) or isinstance(RGB_LED, str)):
            raise TypeError("RGB LED must either be a valid color string or a list of RGB integer values")
        elif isinstance(RGB_LED, list):
            self._verify_RGB_list_input(RGB_LED)
        elif RGB_LED not in self.COLORS:
